...
class TicketStateService {
  Future<SimpleDictionaryModel> createState({
    required String apiKey,
    required String name,
  }) async {
    final uri = Uri.parse('$url/State/NewState');
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      throw Exception('Ошибка создания статуса: ${response.statusCode}');
    }
  }

  Future<SimpleDictionaryModel> updateState({
    required String apiKey,
    required SimpleDictionaryModel state,
  }) async {
    final uri = Uri.parse('$url/State/UpadateStateById')
        .replace(queryParameters: {'id': state.oid.toString()});
    ...
    if (response.statusCode == 200) {
      if (response.body.isEmpty) return state;
      return SimpleDictionaryModel.fromJson(jsonDecode(response.body));
    } else {
  ...
  Future<List<SimpleDictionaryModel>> getAllStates({
    required String apiKey,
  }) async {
    final uri = Uri.parse('$url/State/AllState');
